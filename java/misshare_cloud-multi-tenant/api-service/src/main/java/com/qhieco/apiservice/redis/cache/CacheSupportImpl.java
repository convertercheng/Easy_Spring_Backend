...
import com.qhieco.apiservice.redis.cache.expression.CacheOperationExpressionEvaluator;
import com.qhieco.apiservice.redis.utils.RedisTemplateUtils;
import lombok.Data;
...
public class CacheSupportImpl implements CacheSupport {
    ...
    private static Map<String, BeanReference> beanReferenceMap = new HashMap<>();
    ...
    public void registerInvocation(Object targetBean, Method targetMethod, Class[] invocationParamTypes,
        ...
        final CachedMethodInvocation invocation = new CachedMethodInvocation(key, targetBean, targetMethod, invocationParamTypes, invocationArgs, 0L);
        beanMap.put(targetBean.getClass().getName(), targetBean);
        ...
        for (Cache cache : caches) {
            if (cache instanceof CustomizedRedisCache) {
                ...
                String invocationKey = redisCache.getCacheKey(key);
                redisTemplate.opsForValue().set(getInvocationCacheKey(invocationKey),
                        invocation, redisCache.getExpirationSecondTime(), TimeUnit.SECONDS);
//                invocationMap.put(getInvocationCacheKey(redisCache.getCacheKey(key)), invocation);
                if(beanReferenceMap.get(invocationKey)!=null){
                    BeanReference beanReference = beanReferenceMap.get(invocationKey);
                    beanReference.persistTime = System.currentTimeMillis()/1000L + redisCache.getExpirationSecondTime();
                }else{
                    BeanReference beanReference = new BeanReference(targetBean.getClass().getName(), System.currentTimeMillis()/1000L + redisCache.getExpirationSecondTime());
                    beanReferenceMap.put(invocationKey, beanReference);
                }
            }
        }
    }
    ...
    @Override
    public void deleteCacheBean() {
        HashSet<String> beanName = new HashSet<>();
        Set<String> deleteRef = new HashSet<>();
        beanReferenceMap.forEach((k,v) ->{
            if(v.persistTime>System.currentTimeMillis()/1000){
                beanName.add(v.beanName);
            }else {
                deleteRef.add(k);
            }
        });
        deleteRef.forEach(beanReferenceMap::remove);
        Set<String> deleteBean = new HashSet<>();
        beanMap.forEach((k,v)->{
            if(!beanName.contains(k)){
                deleteBean.add(k);
            }
        });
        deleteBean.forEach(beanMap::remove);
    }
    ...
    private void refreshCache(CachedMethodInvocation invocation, String cacheName) {
        try {
            ...
            BeanReference beanReference = beanReferenceMap.get(redisCache.getCacheKey(invocation.getKey()));
            beanReference.persistTime = System.currentTimeMillis()/1000L + expireTime;
...
//        Object target = ReflectionUtils.getTarget(SpringContextUtils.getBean(Class.forName(invocation.getTargetBean())));
        Object target = beanMap.get(invocation.getTargetBean());
    ...
    private class BeanReference{
        ...
        public BeanReference(String beanName, Long persistTime) {
